...
def render_dnsdist_conf(opts: dict[str, Any]) -> str:
    """Render the dnsdist rate-limit RULES from bundle options (issue #146
    Phase 2). PowerDNS Authoritative has no RRL, so a separate dnsdist front
    container forwards to pdns:53; this renders ONLY the operator's rules
    (MaxQPSIPRule → TC/Drop + optional exceedQRate dynamic block). The dnsdist
    container's entrypoint composes these onto its env-configured base
    (setLocal + newServer → pdns backend), so this is fully decoupled from
    pdns.conf — pdns never moves port. Pure + unit-testable.

    Returns "" when dnsdist is disabled → the front runs as a plain
    pass-through (no rate limiting), which is a safe no-op."""
    if not opts.get("dnsdist_enabled"):
        return ""
    lines = ["-- dnsdist rate-limit rules — generated by SpatiumDDI (issue #146 Phase 2)"]
...
class PowerDNSDriver(DriverBase):
    ...
    def render(self, bundle: dict[str, Any]) -> None:
        ...
        rules = render_dnsdist_conf(opts)
        if rules:
