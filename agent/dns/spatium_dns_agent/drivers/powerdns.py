...
# dnsdist where pdns listens once it's fronted (loopback, shared netns).
DNSDIST_BACKEND = "127.0.0.1:5300"


def render_dnsdist_conf(opts: dict[str, Any], backend: str = DNSDIST_BACKEND) -> str:
    """Render the dnsdist sidecar config from bundle options (issue #146
    Phase 2). PowerDNS Authoritative has no RRL, so dnsdist fronts it on :53
    and forwards to pdns at ``backend``. Pure + unit-testable.

    Emits a per-source-IP QPS cap (MaxQPSIPRule → TC or Drop) and an optional
    dynamic-block rule (exceedQRate). Returns "" when dnsdist is not enabled,
    so the sidecar's watcher tears the front down cleanly."""
    if not opts.get("dnsdist_enabled"):
        return ""
    lines = [
        "-- dnsdist.conf — generated by SpatiumDDI (issue #146 Phase 2)",
        "-- Fronts PowerDNS Authoritative; forwards to the pdns backend.",
        'setLocal("0.0.0.0:53")',
        'addLocal("[::]:53")',
        f'newServer({{address="{backend}", name="pdns"}})',
        'setACL({"0.0.0.0/0", "::/0"})',
    ]
    max_qps = opts.get("dnsdist_max_qps_per_client")
    if max_qps is not None:
        action = "DropAction()" if opts.get("dnsdist_action") == "drop" else "TCAction()"
    ...
    if dyn is not None:
        ...
        lines.append(f'dbr:setQueryRate({int(dyn)}, 10, "exceeded query rate", {secs})')
...
class PowerDNSDriver(DriverBase):
    ...
    def render(self, bundle: dict[str, Any]) -> None:
        ...
        dnsdist_enabled = bool(opts.get("dnsdist_enabled", False))
        ...
        conf_path.write_text(
            self._render_conf(
                ...
                dnsdist_enabled=dnsdist_enabled,
            )
        )

        # dnsdist sidecar config (issue #146 Phase 2). Written to a STABLE path
        # under state_dir (NOT the swapped rendered dir) that the dnsdist
        # sidecar mounts + watches; removed when disabled so the front tears
        # down on the next render.
        dnsdist_path = self.state_dir / "dnsdist.conf"
        dnsdist_conf = render_dnsdist_conf(opts)
        if dnsdist_conf:
            dnsdist_path.write_text(dnsdist_conf)
        elif dnsdist_path.exists():
            dnsdist_path.unlink()
    ...
    def _dnssec_sign(self, api_key: str, zone: str) -> list[str]:
        ...
        headers = {"X-API-Key": api_key, "Content-Type": "application/json"}
        with httpx.Client(timeout=_PDNS_API_TIMEOUT) as client:
            existing = client.get(f"{_PDNS_API_BASE}/zones/{zone}/cryptokeys", headers=headers)
            ...
            after = client.get(f"{_PDNS_API_BASE}/zones/{zone}/cryptokeys", headers=headers)
    ...
    def _rectify(self, client: httpx.Client, headers: dict[str, str], zone: str) -> None:
    ...
    def _render_conf(
        ...
        *,
        ...
        # When a dnsdist sidecar fronts us (#146 Phase 2) pdns moves off :53
        # and listens on loopback:5300 (shared netns) — dnsdist owns :53.
        local_address = "127.0.0.1" if dnsdist_enabled else "0.0.0.0"
        local_port = "5300" if dnsdist_enabled else "53"
        return "\n".join(
            [
