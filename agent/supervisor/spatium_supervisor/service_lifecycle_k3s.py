"""k3s analog of ``service_lifecycle.py`` (#183 Phase 3).

Same public surface (``apply_role_assignment``,
``tear_down_supervised_services``, same ``LifecycleResult`` dataclass)
so ``heartbeat.py`` can dispatch to either backend without caring
which container runtime is live. The dispatcher lives in
``appliance_state.detect_runtime()``: when k3s is up AND the chart
is baked, this module owns orchestration; otherwise the compose
path stays.

Phase 3 design:

* The supervisor doesn't run ``helm`` itself. We construct a
  ``HelmChart`` Custom Resource (the one k3s ships built-in via
  helm-controller) carrying ``spec.chartContent`` (base64-encoded
  tarball) + ``spec.valuesContent`` (rendered YAML) and PATCH it
  into the kubeapi. helm-controller runs ``helm upgrade --install``
...
* Values are derived from the same heartbeat-response shape the
  compose path consumes (``role_assignment`` dict). We translate
  ``COMPOSE_PROFILES`` from the rendered env file → per-role
  ``<role>.enabled: true`` flags + thread the agent keys / group
  names / control-plane URL.
...
from . import k8s_api
from .service_lifecycle import LifecycleResult, SUPERVISED_SERVICES
