...
15. **New integrations show up on the Dashboard**: When adding an integration mirror (Kubernetes / Docker / Proxmox / Tailscale / UniFi shape — read-only pull reconciler with per-target rows), the new integration MUST also be wired into the `IntegrationsPanel` on `frontend/src/pages/DashboardPage.tsx`: (a) import the API client + row type, (b) add a `useQuery` gated on the `integration_*_enabled` flag with 30 s `refetchInterval`, (c) thread the `enabled` flag + row list through the `IntegrationsPanel` props, (d) extend the column-count calc + `cn()` grid case for the new total, (e) add a panel block inside the grid following the existing pattern (icon + name + count + view-all link + per-row `IntegrationRow` with `IntegrationDot` health indicator). Operators expect the dashboard to surface every connected target — a new integration that doesn't appear there is invisible to anyone who doesn't already know the sidebar entry exists
