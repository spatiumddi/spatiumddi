...
| `ntp_servers` | string or list, default `pool.ntp.org`. Written to `/etc/chrony/sources.d/spatium-install.sources`; **empty is a valid answer** (air-gapped site with a trusted RTC). Superseded and removed if the control plane later manages NTP (#154) |
