...
| `keymap` | console keymap for `loadkeys`, default `us`. Persisted to both `/etc/default/keyboard` and `/etc/vconsole.conf`. Shape-checked here; a name the target cannot load is caught on the live system, which is the only place that can know |
