...
| `ntp_servers` | string or list, default `pool.ntp.org`. Written to `/etc/chrony/sources.d/spatium-install.sources`; **empty writes no file**, which is not the same as no time source — Debian's `chrony.conf` keeps its own `pool` directive and sources.d is additive, so the public pool still applies (#1002). Superseded and removed if the control plane later manages NTP (#154) |
