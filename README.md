...
- **OS Image (atomic A/B upgrade)** — Phase 8 (issue [#138](https://github.com/spatiumddi/spatiumddi/issues/138)). The installer carves two equal-sized root partitions (`root_A` + `root_B`); the appliance always boots one slot while the other sits idle. Pick a release tag from the dropdown (sourced from GitHub Releases) — the slot `.raw.xz` + sha256 URLs derive automatically. Apply writes to the *inactive* slot via dd while the active slot is untouched, arms grub `next_entry` as one-shot, and on the next reboot `/health/live` confirms before grub-set-default commits the swap. Bad image? Reboot reverts automatically. Rollback button flips the durable default back to the previous slot from the same card (no SSH); Reboot-now button on the success banner avoids the trip to Maintenance. Operator state on `/var/persist/etc`, `/var/home`, `/var/root`, and the entire Docker image cache lives on a third persistent `/var` partition that's shared across slots, so a slot swap can't lose user data.
- **Fleet upgrade** — Phase 8f (issue [#138](https://github.com/spatiumddi/spatiumddi/issues/138)). New top-level Fleet tab in `/appliance` lists every registered DNS + DHCP agent with its slot state, deployment kind, installed version, and any pending operator-set desired version. Per-row Upgrade button stamps the picked release tag on the agent's server row; the agent's ConfigBundle long-poll picks it up (~30–60 s) and fires the same slot-upgrade trigger the per-box flow uses. Docker / k8s agents get a Manual upgrade modal with copy-paste `docker compose pull && up -d` / `helm upgrade … --set image.tag=…` commands instead, since they have no A/B partition to dd into. Auto-clear on success — the pending chip drops as soon as the agent reports the matching installed version.
