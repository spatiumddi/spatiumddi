...
| 🗓 | **Scheduled Wake-on-LAN** | **Tools → Wake Schedules** — recurring DST-safe cron wake of a whole fleet, targeted by address tags / subnet / subnet tags / explicit hosts · holiday gate (blackout dates + term range) or a subscribed **iCal / CalDAV calendar** (skip-on-event or only-on-event) · **post-wake liveness verify** stamps responders into the `last_seen_at` signal and re-wakes only the hosts that didn't come up · MAC fallback via `ip_mac_history` → DHCP lease · hard fan-out cap + auto-tuned stagger · togglable `tools.wake_scheduler` module |
